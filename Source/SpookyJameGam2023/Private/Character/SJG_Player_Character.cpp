...
ASJG_Player_Character::ASJG_Player_Character()
{
	...
	SpringArmComponent->SetupAttachment(RootComponent);  // or another suitable component
	SpringArmComponent->bUsePawnControlRotation = true;  // Make sure this is set to true
	...
	InteractionCheckFrequency = 0.15f;
	...
	for (FHitResult Hit : Hits)
	{
		if (AActor* HitActor = Hit.GetActor())
		{
			...
			if (HitActor->GetClass()->ImplementsInterface(UIsACollectable::StaticClass()))
			{
				...
				UE_LOG(LogTemp, Warning, TEXT("interact from perform interact"));
			}
			else
			{
